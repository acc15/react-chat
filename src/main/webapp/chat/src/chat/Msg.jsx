...
class Msg extends React.Component {
    ...
    render() {
        ...
        return <div className={classnames("msg", "msg-" + msg.type)} style={{opacity: this.state.live}}>
            <span className="time">{moment(msg.time).format('DD.MM.YYYY HH:mm:ss.SSS')}</span> { msg.user && <span className={classnames("user", user.id === msg.user.id && "me")}>{msg.user.name}</span> } <ReactMarkdown source={msg.text}/>
